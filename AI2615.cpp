...
//9.30 佳祺的代码（更改后）
/*
void merge(vector<int>& arr, int start, int end, vector<int>& ans) {
    int leftE = (start + end) / 2;
    int rightS = ((start + end) / 2) + 1;
    
    vector<int> leftArr;
    vector<int> rightArr;

    for (int i = start; i <= leftE ; ++i)
        leftArr.push_back(arr[i]);

    for (int i = rightS; i <=end; ++i)
        rightArr.push_back(arr[i]);

    int leftIndex = 0, rightIndex = 0, cnt = 0;

    for (int i = start; i <= end; ++i) {
        if (leftIndex < rightS - start && rightIndex < end - leftE) //这里的两个size()都是可以计算出来的
        {
            ...
            else 
            {
                ...
                ans[arr[i]] += rightS - start - leftIndex; //这里是改动的地方，其中size()可以被计算出来
            }
        }
        else if (leftIndex == leftArr.size()) 
        {
            arr[i] = rightArr[rightIndex++]; //这里是改动的地方
        }
        else
        {
            arr[i] = leftArr[leftIndex++];
...
void mergeSort(vector<int>& arr, int start, int end, vector<int>& ans) {
    if (end > start) 
    {
        mergeSort(arr, start, (start + end) / 2, ans);
        mergeSort(arr, (start + end) / 2 + 1, end, ans);
        merge(arr, start, end, ans);
    }
}
